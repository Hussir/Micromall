	...
	var page = {
		init:function(){
			...
			$('uploadType').onclick = function(e){
				...
				if(o.nodeName==="INPUT"){
					...
					picture.classList.remove("z-err");
			...
			$('upload').addEventListener('click', function (){
				...
				var file = uploadInput.files[0];
					...
					xhr.open("post", "/goods/api/upload", true);
		            xhr.onload = function () {
						if (xhr.status === 200) {
						alert("文件上传成功");
			...
			[title,summary,picture,description,price].forEach(function(item){
				item.addEventListener('input',function(e){
					item.classList.remove('z-err');
				}.bind(this),false);
			}.bind(this));
            picture.addEventListener('input',function(e){
				var value = picture.value.trim();
				if(value != ''){
					imgpre.src = value;
				}
			}.bind(this),false);
		...
		check:function(){
			...
			[
				...
				[picture,function(value){return imageMode == "urlUpload" && value == ''}],
				[description,function(value){return value.length<2 || value.length>1000}],
